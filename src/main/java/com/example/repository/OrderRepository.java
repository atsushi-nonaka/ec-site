...
public class OrderRepository {
	...
	/**
	 * 注文情報をDBから検索しセットするローマッパー.
	 */
	public final static RowMapper<Order> ORDER_ROW_MAPPER = (rs, i) -> {
	...
	/**
	 * CSVをダウンロードするためのResultSetExtractor.
	 */
	private final static ResultSetExtractor<List<Order>> CSV_RESULT_SET_EXTRACTOR = (rs) -> {
	...
	/**
	 * ショッピングカートの中身を表示させるResultSetExtractor.
	 */
	...
	/**
	 * idカラムが自動採番されることをDIコンテナに教える. init()...インスタンスされるときに1度だけ実行されるメソッド.
	 */
	@PostConstruct
	public void init() {
	...
	public void update(Order order) {
		String sql = "UPDATE orders SET order_date = :orderDate, user_id = :userId, status = :status, destination_name = :destinationName, destination_email = :destinationEmail, "
	...
	/**
	 * CSV出力するために必要な情報（order. orderItem）を取得する.
	 * 
	 * @return 注文情報リスト
	 */
	public List<Order> findCsvData() {
		String sql = "SELECT o.order_number o_order_number, o.user_id o_user_id, o.status o_status, o.total_price o_total_price, o.order_date o_order_date, "
				...
				+ "FROM orders o LEFT OUTER JOIN order_items oi ON o.order_number = oi.order_number WHERE NOT status = :status";

		SqlParameterSource param = new MapSqlParameterSource().addValue("status", 0);
		List<Order> orderList = template.query(sql, param, CSV_RESULT_SET_EXTRACTOR);
	...
	public List<Order> findOrder() {
		String sql = "SELECT o.order_number o_order_number, o.user_id o_user_id, o.status o_status, o.total_price o_total_price, o.order_date o_order_date, "
				...
				+ "WHERE NOT status = :status ORDER BY i.price_m";
	...
	public List<Order> findOrderHistory(Integer userId, boolean isJoin) {
		String sql = null;
		if (isJoin) {
			sql = "SELECT o.order_number o_order_number, o.user_id o_user_id, o.status o_status, o.total_price o_total_price, o.order_date o_order_date, "
					+ "o.destination_name o_destination_name, o.destination_email o_destination_email, o.destination_zipcode o_destination_zipcode, "
					+ "o.destination_address o_destination_address, o.destination_tel o_destination_tel, o.delivery_time o_delivery_time, o.payment_method o_payment_method, "
					+ "u.id u_id, u.name u_name, u.email u_email, u.password u_password, u.zipcode u_zipcode, u.address u_address, u.telephone u_telephone, u.admin u_admin, "
					+ "oi.id oi_id, oi.item_id oi_item_id, oi.order_number oi_order_number, oi.quantity oi_quantity, oi.size oi_size, "
